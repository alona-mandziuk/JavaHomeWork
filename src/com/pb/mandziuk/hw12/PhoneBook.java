...
public class PhoneBook {
    ...
    private int getNextId() {
        if (!contacts.isEmpty()) {
            Contact maxIdContact = Collections.max(contacts, Comparator.comparingInt(Contact::getId));
    ...
    public void findContactsByName() {
        ...
        String name = scan.nextLine();
        List<Contact> foundContacts = contacts.stream().filter(c -> c.getName().contains(name)).collect(Collectors.toList());
    ...
    public void findContactsByPhone() {
        ...
        List<Contact> foundContacts = contacts.stream().filter(c -> c.getPhoneNumbers().stream().anyMatch(number ->
                number.contains(phonePart))).collect(Collectors.toList());
        printContacts(foundContacts);
    }
    ...
    public void sortByChangeDateAndPrint() {
        contacts.sort(((Comparator<Contact>) (c1, c2) -> c1.getChangeTime().compareTo(c2.getChangeTime())).reversed());
        printContacts(contacts);
    }
    ...
    private void editPhoneNumbers(Contact contact) {
        ...
        while (true) {
            ...
            IntStream.range(0, phoneNumbers.size()).mapToObj(i -> (i + 1) + ". " + phoneNumbers.get(i)).forEach(System.out::println);
